...
for year in sorted(pub_strs["worksinprogress"].keys(), reverse=True):
    ...
    for key, pub in pub_strs["worksinprogress"][year]:
        ...
        if notes:
            # Use the markdown function to render the notes
            notes_html = markdown(notes)
            pub += f"<ul><li style='font-size: smaller;'>{notes_html}</li></ul>"
        pub += "</li>"  # Move this line up here
        bolded_publications.append(pub.replace('Hussain Hadah', '<b>Hussain Hadah</b>'))
    display(HTML(
        "<ul class='list-group list-group-flush'>" + '\n'.join(bolded_publications) + "</ul>"
    ))
```
